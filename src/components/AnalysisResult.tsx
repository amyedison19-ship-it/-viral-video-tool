...
export default function AnalysisResult({ analysis, onReset }: Props) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Tabs */}
      <div className="px-6 flex gap-6 border-b" style={{ borderColor: 'var(--border-color)' }}>
        {tabs.map(tab => (
          <button
            key={tab.key}
            className={`pb-3 px-1 text-sm font-medium transition-colors flex items-center gap-1.5 ${
